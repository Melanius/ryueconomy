...
export async function getPageContentAndThumbnail(pageId: string): Promise<{ content: string, thumbnail: string, image: string }> {
  ...
  try {
    ...
    console.log(`📊 샘플 블록 구조:`, JSON.stringify(sampleBlocks.map(block => ({
      ...
      has_children_in_data: !!(block as any).children && (block as any).children.length > 0
    })), null, 2));
    ...
    const countBlockTypes = (blocks: any[]) => {
      blocks.forEach(block => {
        ...
        blockTypeStats[type] = (blockTypeStats[type] || 0) + 1;
        
        // 하위 블록이 있으면 재귀적으로 카운트
        if (block.children && block.children.length > 0) {
          countBlockTypes(block.children);
        }
      });
    ...
    const findFirstImage = (blocks: any[]): string => {
      for (const block of blocks) {
        if (block.type === 'image' && block.image && block.image.file?.url) {
          return block.image.file.url;
        } else if (block.type === 'image' && block.image && block.image.external?.url) {
          return block.image.external.url;
        } else if (block.children && block.children.length > 0) {
          const childImage: string = findFirstImage(block.children);
    ...
    image = findFirstImage(blocks);
    thumbnail = image; // 기본값으로 첫 번째 이미지 사용
    
    // 커버 이미지가 있으면 그것을 우선적으로 사용
    try {
    ...
    try {
      ...
      const { html: content } = await renderBlocks(blocks);
...
export async function getPostBySlug(slug: string): Promise<BlogPost | null> {
  try {
    ...
    // 컨텐츠 및 썸네일 가져오기
    const { content, thumbnail } = await getPageContentAndThumbnail(page.id);
    post.content = content;
...
export const getRelatedPosts = cache(async (currentPostId: string, categoryName: string): Promise<BlogPost[]> => {
  ...
  return posts
    .filter((post) => post.id !== currentPostId && post.category === categoryName)
    .slice(0, 3);
});
    ...
    if (response.results.length > 0) {
      ...
      console.log(JSON.stringify({
        ...
        properties: Object.keys(firstPage.properties).map(key => ({
          name: key,
          type: (firstPage.properties as any)[key].type
        }))
      }, null, 2));
    }
...
function cleanHtml(html: string): string {
  ...
  const patterns = [
    ...
    /<\/div><\/li>/g,
    /<\/li><\/div>/g,
    /<\/div>(?!\s*<)/g,  // div 태그가 실제 태그가 아니라 텍스트로 표시된 경우 제거
